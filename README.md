...
- **Tauri アプリケーション**: サーバー機能とイベント管理・モニタ機能
  - Socket.IO サーバー (ポート 12345)
  - HTTP サーバー (ポート 8080)
  - イベント作成・管理 UI
  - リアルタイムモニタ画面
- **ブラウザベース出席登録ページ**:
  - 任意のブラウザから `http://[IP]:8080/attendance.html` にアクセス
  - 学籍番号入力による出席登録
  - リアルタイムで出席状況を表示
...
- **Socket.IO サーバー**: リアルタイムデータ同期（ポート 12345）
- **HTTP サーバー**: 静的ファイル配信（ポート 8080）
...
1. 任意のブラウザで出席登録ページにアクセス
   - URL: `http://[IPアドレス]:8080/attendance.html?uuid=[イベントID]&server=[IPアドレス]:12345`
